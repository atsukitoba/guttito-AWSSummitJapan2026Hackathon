...
**プロダクト**: 本プロダクト「gutitto」は「ぐっとくる」「気持ちがこもる」「吐き出す（ぐち）」を掛けた造語で、疲れた夜に "ぐちる" ように安心して何でも話せるダメなあなたになれる体験を提供します。

---

## 何をするプロダクトか

**この現代を生きる頑張る社会人に、「あなたがダメになれる存在」を提供する。**

gutittoはあなたを世界で一番理解している AI ペルソナです。人に言いづらい感情、思い、考えを吐き出すことができます。あなたは整理も気合も要らず、ぽつぽつ話して寝るだけ。会話後はAIペルソナが裏で内容を分析し、仕事の報連相・タスク・やりたいことを自動整理して明日を頑張るためのヒントを投稿してくれます。

「**ダメになれる存在** がそばにいる安心感」で感情と行動の両方を off-load するサービスであり、
受容 × ゼロ UX × 予習知の三層プロダクトです。
...
ロック画面のウィジェットに AI ペルソナの顔が常駐しており、押し付けがましい通知は出さず、**静かな存在感** だけを放ち、ユーザーは無意識にその存在を視界の隅で感じ、いつでも "呼べる" 安心感の中で過ごします。
...
ウィジェットをタップした瞬間、ペルソナから先に話しかけてくる。**ユーザーは最初の一言を考える必要すらありません**。
...
### 3. 通話後（Post-call）— "ただ愚痴るだけ" じゃない

愚痴が終わったら、ユーザーは寝るだけ。整理は AI が裏側で行います。
...
gutitto は **3 本柱** を積層的に備えることで成立します。どれか 1 つが欠けても「ダメになれる」体験は成立しません。

| # | 柱 | 内容 |
|---|---|------|
| 1 | 🫂 **受け止める力（Accepting Power）** | 話す内容の整理や気持ちを使う必要なしの受容 |
| 2 | ⚡ **ゼロ UX（Zero-UX）** | 頭も心も身体も使わせない起動体験。ロック画面から 1 タップ |
| 3 | 🧠 **あなたを知っていること（You-Know-Me）** | Google カレンダー・Slack・Gmail・過去会話を予習エージェントがあなたを徹底的に理解。知っているから話したくなる |
...
4. **各 Unit 間のインターフェースが明確** — 結合度低く保つ
...
プロダクトを体現する5本のMVP（プライマリペルソナ「まなみ」視点）:

| ID | ユーザーストーリー | 担当 Unit |
|----|---------|---------|
| **1.1** | スマホを触る気力もない夜でも、ロック画面から 1 タップで話し始めたい。 | U1 |
| **2.1** | 通話した瞬間、AI から先に話しかけてほしい。沈黙も許してほしい。 | U1 + U2 + U3 |
| **2.2** | 整理されていない愚痴も、否定せず受け止めてほしい。 | U2 |
| **2.3** | 黙っていても、AI が話題を振るか、相槌だけで寄り添ってほしい。 | U2 + U3 |
| **4.1** | カレンダー・Slack・Gmail・過去会話から、私を知っていてほしい。 | U3 |

全 15 ストーリーと受け入れ基準は [`stories.md`](./aidlc-docs/inception/user-stories/stories.md) を参照。
...
| 役割 | 担当メンバー | 担当 Unit |
|-----|----------|---------|
| 代表 / 開発 UI / プレゼン / ビジネスデザイン | 代表 |U1 入口 Unit|
